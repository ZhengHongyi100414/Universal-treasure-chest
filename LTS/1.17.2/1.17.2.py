...
def change_api_key():
    global api_key
    password=gui.passwordbox(msg='请输入密码', title='更改API Key', default='你猜密码是多少', image=None, root=None)
...
while True:
    ...
    if temp=='1':
        ...
        break
    '''
    temp=gui.choicebox(msg='请选择您需要的服务', title='超级百宝箱 22.1123A', choices=['单位换算','个人所得税计算','货币转换','时钟','天气','IP地址查询','文件朗读','虚拟身份生成','二维码生成','笑一笑','更改API Key','exit'])
    print(temp)
    if temp=='单位换算':
        danwei()
    if temp=='个人所得税计算':
