...
def weather():
    ...
    url='http://api.airvisual.com/v2/countries?key=9677ca14-c723-4a20-9d58-e0a1d2413958'
    payload={}
    headers = {}
    response = requests.request("GET", url, headers=headers, data=payload)
    a=response.json()
    ...
    url='http://api.airvisual.com/v2/states?country='+cou+'&key=9677ca14-c723-4a20-9d58-e0a1d2413958'
    payload={}
    headers = {}
    response = requests.request("GET", url, headers=headers, data=payload)
    a=response.json()
    ...
    url='http://api.airvisual.com/v2/cities?state='+sta+'&country='+cou+'&key=9677ca14-c723-4a20-9d58-e0a1d2413958'
    payload={}
    headers = {}
    response = requests.request("GET", url, headers=headers, data=payload)
    a=response.json()
    ...
    url = "http://api.airvisual.com/v2/city?city="+cit+"&state="+sta+"&country="+cou+"&key=9677ca14-c723-4a20-9d58-e0a1d2413958"
    payload={}
    headers = {}
    response = requests.request("GET", url, headers=headers, data=payload)
    a=response.json()
...
while True:
    ...
    if temp=='1':
        ...
        break
    '''
    temp=gui.choicebox(msg='请选择您需要的服务', title='超级百宝箱 22.1120A', choices=['单位换算','个人所得税计算','货币转换','时钟','天气','IP地址查询','文件朗读','虚拟身份生成','二维码生成','笑一笑','exit'])
    print(temp)
    if temp=='单位换算':
        danwei()
    if temp=='个人所得税计算':
