...
def ip():
    ...
    gui.msgbox(msg='当前ip地址为'+dictr['ip'], title='ip地址', ok_button='确定', image=None, root=None)
while True:
    '''
    ...
    if temp=='1':
        ...
        break
    '''
    temp=gui.choicebox(msg='请选择您需要的服务', title='超级百宝箱 1.8.8', choices=['单位换算','个人所得税计算','货币转换','时钟','天气','IP地址查询','exit'])
    print(temp)
    if temp=='单位换算':
        danwei()
    if temp=='个人所得税计算':
